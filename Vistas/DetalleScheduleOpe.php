    ...
    <div class="wrapper">
           ...
           <ul class="sidebar-menu">
            ...
            <li class="active treeview">
              ...
              <ul class="treeview-menu">
                  ...
                  <ul class="treeview-menu">
                      <li class="active"><a href="DetalleScheduleOpe.php"><i class="fa fa-circle-o"></i> Detalle de Schedule </a></li>                    
                  </ul>
                  <ul class="treeview-menu">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            DETALLE DE SCHEDULE
            <small>ACTIVIDADES DEL SCHEDULE</small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="index.php"><i class="fa fa-user"></i> SCHEDULE</a></li>
            <li><a href="index.php">Ejecutar Schedule</a></li>
        ...
        <form class="form-horizontal" id="form_cerrar_schedule" action="../Controles/Registro/CSchedule.php" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-12">
                                <div class="box box-info">
                                    <div class="box-header with-border">
                                      <h3 class="box-title">DATOS DEL SCHEDULE</h3>
                                    </div><!-- /.box-header -->
                                    <!-- form start -->
                                   <div class="box-body">
             ...
             <div class="box box-info">
                 ...
                 <div class="box-body">
                    <div class="form-group">
                      ...
                      <div class="col-sm-10">
                        <input type="text" class="form-control" name="t_descripcion" placeholder="Ingrese Descripción">
                      </div>
                    </div> 
    ...
    foreach ($actividades as $a) {
        ?>
                      <tr style="font-size:10pt;" <?php if($a['actividad_tws']=='1')echo 'bgcolor="68FF7E"';?>>
                        ...
                        <td style="color:black;font-weight:bold;"><?php echo $a['actividad_descripcion'] ?></td>  
                        <td align="center">
                           <?php if ($a['procedimiento_nombre']!='NO TIENE') {?> 
                   ...
                   <?php echo $a['actividad_servidor'] ?>
                  </div>
                            
                        </td>                        
                        <td style="font-weight:bold;">
                                                                       
                                                         <div id="inicio_tar<?php echo $a['schedact_idschedact'] ?>">
                                                         <?php if($a['schedact_horaini'] == '') { ?>
                                                            <input type="hidden" name="id_schedule_act" id="id_schedule_act<?php echo $a['schedact_idschedact'] ?>" value="<?php echo $a['schedact_idschedact'] ?>">
                                                            <input type="hidden" name="hidden_schedule" id="hidden_schedule" value="iniciar_tarea">
                                                            <button type="submit" class="btn btn-primary btn-xs" onclick="iniciarTarea('<?php echo $a['schedact_idschedact'] ?>');" title="Iniciar Tarea"><i class="fa fa-clock-o"> Iniciar</i></button>
                                                          <?php } else { ?>  
                                                              <div class="alert alert-warning"><?php echo date('H:i:s',strtotime($a['schedact_horaini']))?></div> 
                                                          <?php } ?>
                                                        </div>
                        </td>
                        <td  style="font-weight:bold;">
                                                         <div id="div_finalizar_tarea<?php echo $a['schedact_idschedact'] ?>">
                                                            
                                                        </div>
                                                        
                                                        <?php if($a['schedact_horaini'] != '') { ?>
                                                            <?php if($a['schedact_horafin'] == '') { ?>
                                                        
                                                              <div id="div_finalizatarea<?php echo $a['schedact_idschedact'] ?>">
                                                                    <button type="button" onclick="finalizar_Tarea('<?php echo $a['schedact_idschedact'] ?>')" 
                                                                            class="btn btn-warning btn-xs"  title="Finalizar Tarea"><i class="fa fa-clock-o"> Finalizar</i></button>
                                                              </div>
                                                        
                                                            <?php } ?>
                                                         <?php if($a['schedact_horafin'] != '') { ?>
                                                         <div class="alert alert-success"><?php echo date('H:i:s',strtotime($a['schedact_horafin']))?></div> 
                                                         <?php } ?>
                                                         <?php } ?>
                        </td>
                        <td  style="font-weight:bold;">
                            <div id="div_comentario_tarea<?php echo $a['schedact_idschedact']; ?>">
                                                        
                                                        </div>
                                                        
                                                        <?php if($a['schedact_horafin'] != '' && $a['schedact_horaini'] !='' ) { ?>
                                                        
                                                            <!--<form method='POST' action="../Controles/Registro/CSchedule.php" >-->
                                                            <textarea name="txt_comentario" id="txt_comentario<?php echo $a['schedact_idschedact'] ?>"><?php if($a['schedact_comentario']!=''){ echo $a['schedact_comentario']; }?></textarea>
                                                            <input type="hidden" name="horainicio" id="horainicio<?php echo $a['schedact_idschedact'] ?>" value="<?php echo date('H:i:s',strtotime($a['schedact_horaini']))?>">
                                                            <input type="hidden" name="horafinal" id="horafinal<?php echo $a['schedact_idschedact'] ?>" value="<?php echo date('H:i:s',strtotime($a['schedact_horafin']))?>">
                                                            <select class="form-control" name="c_estado_tar" id="c_estado_tar<?php echo $a['schedact_idschedact'] ?>" required style="font-size:5pt;">                                                            
                                                                <option value="1" <?php if($a['schedact_estado_tar']=='1') echo 'selected';?>>OK</option>
                                                                <option value="2" <?php if($a['schedact_estado_tar']=='2') echo 'selected';?>>FALLIDO</option>
                                                                <option value="3" <?php if($a['schedact_estado_tar']=='3') echo 'selected';?>>NA</option>
                                                            </select>
                                                            <br>
                                                            <button type="button"
                                                                    onclick="comentario_Tarea('<?php echo $a['schedact_idschedact'] ?>')"
